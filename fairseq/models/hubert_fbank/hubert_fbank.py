...
@dataclass
class HubertFbankConfig(FairseqDataclass):
    ...
    freq_mask_F: int = field(
        default=30
    )
    freq_mask_N: int = field(
        default=2
    )
    time_mask_N: int = field(
        default=2
    )
    time_mask_T: int = field(
        default=40
    )
    time_mask_p: float = field(
        default=1.0
    )
    time_wrap_W: int = field(
        default=0
    )


@register_model("hubert_fbank", dataclass=HubertFbankConfig)
class HubertFbankModel(BaseFairseqModel):
    def __init__(
        self,
        cfg: HubertFbankConfig,
        task_cfg: HubertPretrainingConfig,
        dictionaries: List[Dictionary],
    ) -> None:
        super().__init__()
        logger.info(f"HubertFbankModel Config: {cfg}")
    ...
    @classmethod
    def build_model(cls, cfg: HubertFbankConfig, task: HubertPretrainingTask):
        """Build a new model instance."""

        model = HubertFbankModel(cfg, task.cfg, task.dictionaries)
        return model
    ...
    def forward_features(self, source: torch.Tensor, padding_mask: torch.Tensor) -> torch.Tensor:
        _, src_lengths = torch.max(padding_mask, dim=1)
        for i in range(len(src_lengths)):
            if src_lengths[i] == 0:
                src_lengths[i] = source.shape[1]
                
        # for i in range(len(src_lengths) - 1):  
        #     if src_lengths[i] == src_lengths[i + 1]:
        #         continue
        #     else:
        #         print(src_lengths)
        #         input('wait\n')
        ...
        if self.feature_grad_mult > 0:
            features, enc_len = self.feature_extractor(source, src_lengths)
            if self.feature_grad_mult != 1.0:
                features, enc_len = GradMultiply.apply(features, self.feature_grad_mult)
        else:
            with torch.no_grad():
                features, enc_len = self.feature_extractor(source, src_lengths)
        return features
    ...
    def extract_features(
        ...
        return res['features'], res["x"], res['layer_results'], res["padding_mask"]     # WYJ modified
...
class FbankExtractor(nn.Module):
    def __init__(self, conv_channels, encoder_embed_dim, conv_kernel_sizes, dropout, encoder, cfg: HubertFbankConfig) -> None:
        super().__init__()
        ...
        self.specaug_config = {
            "freq_mask_F": cfg.freq_mask_F, 
            "freq_mask_N": cfg.freq_mask_N, 
            "time_mask_N": cfg.freq_mask_N, 
            "time_mask_T": cfg.time_mask_T,
            "time_mask_p": cfg.time_mask_p, 
            "time_wrap_W": cfg.time_wrap_W
        }
        from fairseq.data.audio.feature_transforms.specaugment import SpecAugmentTransform
        self.specaug_transform = SpecAugmentTransform.from_config_dict(self.specaug_config)
    ...
    def extract_fbank_features(self, source, src_lengths, apply_specaug):
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(source.size(0)):
                ...
                if self.specaug and apply_specaug is True:
